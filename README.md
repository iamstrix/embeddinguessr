...
How to Play
Daily mode — A new puzzle every day, same word for everyone worldwide. Compete on the leaderboard by solving it in as few guesses as possible.
Endless mode — Play unlimited puzzles, one after another, any time. Logged-in players can submit scores to the endless leaderboard, tracked by average guesses per game.
Each guess:
...
Architecture
artifacts/
  embedding-wordle/   # React + Vite frontend (3D game)
  api-server/         # Express API (embeddings, game logic, auth)
lib/
  db/                 # Drizzle ORM schema + migrations
  api-client-react/   # Generated React Query hooks from OpenAPI spec
  api-spec/           # OpenAPI 3.1 contract + Orval codegen config

The API is contract-first: every endpoint is defined in OpenAPI, and the client-side hooks are automatically generated. The embedding model loads once at server startup and handles all similarity computations in-process — no external AI API calls, no rate limits, no costs per guess.
...
# Start API server (loads embedding model on startup, ~5-10s)
pnpm --filter @workspace/api-server run dev
# Start frontend
pnpm --filter @workspace/embedding-wordle run dev

Set DATABASE_URL and VITE_CLERK_PUBLISHABLE_KEY in your environment. Clerk sign-in requires localhost for the dev key, or a published domain for the live key.
